...
void saveWithMmap(const string &file_name) {
#ifdef TEST
    ...
    int res_size = 0;
    char c[8];
    int start = toCharArray(circle_sum, c);
    c[start] = '\n';
    res_size += (++start);

    for (int i = 0; i < 5; ++i) {
        res_size += resByteNum[i];
    }

    int fd = open(file_name.c_str(), O_RDWR | O_CREAT, 00777);
    lseek(fd, res_size - 1, SEEK_END);
    write(fd, " ", 1);
    char *buffer = (char *) mmap(NULL, res_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);

    memcpy(buffer, c, start);
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        memcpy(buffer + start, res[i], resByteNum[i]);
        start += resByteNum[i];
    }

#ifdef TEST
    chrono::duration<double, std::milli> duration = chrono::system_clock::now() - start_time;
    cout << "write file use: " << duration.count() << "ms" << endl;
    cout << "circle_sum: " << circle_sum << endl;
    cout << "bytes: " << res_size << endl;
#endif

    munmap(buffer, res_size);
}
