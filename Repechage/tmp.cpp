...
struct Path {
    ...
    Path(int u2, int u1, int w2, int w1) : u2(u2), u1(u1), w2(w2), w1(w1) {}
...
vector<unordered_map<int, vector<Path>>> pathDetail;
    ...
    switch (depth) {
        case 1:
            for (int i = 0; i < (int) graph[cur].size(); i += 2) {
                ...
                dfs_1(visit, head, v, w3, depth + 1);
            }
            break;
        case 2:
            for (int i = 0; i < (int) graph[cur].size(); i += 2) {
                int v = graph[cur][i], w3 = graph[cur][i + 1];
                if (v < head || visit[v] || !check(w, w3)) continue;
                isValid[cur] = head;
                dfs_1(visit, head, v, w3, depth + 1);
            }
            break;
    ...
    switch (depth) {
        case 1:
            for (int i = 0; i < (int) graphIn[cur].size(); i += 2) {
                int u = graphIn[cur][i], w3 = graphIn[cur][i + 1];
                if (u < head || visit[u]) continue;
                dfs_2(visit, head, cur, u, w3, w3, depth + 1);
            }
            break;
        case 2:
            for (int i = 0; i < (int) graphIn[cur].size(); i += 2) {
                int u = graphIn[cur][i], w3 = graphIn[cur][i + 1];
                if (u < head || visit[u] || !check(w3, w)) continue;
                isValid[cur] = head;
                dfs_2(visit, head, cur, u, w1, w3, depth + 1);
            }
            break;
        case 3:
            for (int i = 0; i < (int) graphIn[cur].size(); i += 2) {
                int u = graphIn[cur][i], w3 = graphIn[cur][i + 1];
                if (u < head) continue;
                if ((u == head || !visit[u]) && check(w3, w)) {
                    isValid[cur] = head;
                    isValid[u] = head;
                    isTail[u] = head;
                    pathDetail[head][u].emplace_back(Path(cur, father, w3, w1));
                }
            }
            break;
    }
    ...
    for (int i = 0; i < (int) graph[cur].size(); i += 2) {
        ...
        if (isTail[v] == head) {
            for (int j = 0; j < (int) pathDetail[head][v].size(); ++j) {
                ...
                int u2 = detail.u2, u1 = detail.u1, w2 = detail.w2, w1 = detail.w1;
