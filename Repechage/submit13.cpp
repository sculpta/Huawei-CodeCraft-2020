...
void createGraph(const char *buffer) {
    ...
    cout << "input_sum: " << (input_sum >> 1) << endl;
#endif
...
void dfs_2(bool *visit, int *isValid, int *isTail,
    ...
    switch (depth) {
        case 1:
            for (int i = 0; i < (int) graphIn[cur].size(); i += 2) {
                ...
                dfs_2(visit, isValid, isTail, pathDetail, head, u, w1, depth + 1);
            }
            break;
        case 2:
            for (int i = 0; i < (int) graphIn[cur].size(); i += 2) {
...
void dfs(bool *visit, int *isValid, int *isTail, vector<unordered_map<int, vector<int>>> &pathDetail,
    ...
    for (int i = 0; i < (int) graph[cur].size(); i += 2) {
        ...
        if (depth < 5)
            dfs(visit, isValid, isTail, pathDetail, head, v, w0, w3, depth + 1, path, tr);
    }
...
void threadFunc(int tid, int start, int end, ThreadResult *tr) {
    ...
    for (int i = start; i < end; ++i) {
        if (!graph[i].empty() && !graphIn[i].empty()) {
            ...
            dfs_2(visit, isValid, isTail, pathDetail, i, i, -1, 1);
            dfs(visit, isValid, isTail, pathDetail, i, i, -1, -1, 1, path, tr);
        }
    }

#ifdef TEST
    chrono::duration<double, milli> duration = chrono::system_clock::now() - start_time;
    cout << "thread " << (tid + 1) << " use: " << duration.count() << "ms" << endl;
#endif
}
...
void work() {
    ...
    int arg1 = 0.055 * node_sum, arg2 = 0.13 * node_sum, arg3 = 0.43 * node_sum;
    int arg[] = {0, arg1, arg2, arg3, node_sum - 2};

#ifdef TEST
    for (int i = 0; i < 4; ++i) {
        cout << "thread " << (i + 1) << " nodes: [" << arg[i] << ", " << arg[i + 1] << ")" << endl;
    }
#endif

    for (int i = 0; i < THREAD_COUNT; ++i) {
        td[i] = thread(&threadFunc, i, arg[i], arg[i + 1], &ans[i]);
    }
    for (auto &t : td) {
        t.join();
    }
