    ...
    for (; it != graph[cur].end(); ++it) {
        ...
        if (isTail[*it] == head) {
            for (int u1 : pathDetail[head][*it]) {
                ...
                temp.push_back(*it);
                temp.push_back(u1);
                res[depth - 1].emplace_back(Path(depth + 2, temp));
                ++circle_sum;
#ifdef TEST
                ++resNum[depth - 1];
#endif
...
void save(const string &file_name) {
    ...
    for (int i = 0; i < 5; ++i) {
#ifdef TEST
        cout << (i + 3) << " circle: " << resNum[i] << endl;
#endif
        for (auto item : res[i]) {
            auto path = item.path;
            int size = path.size();
            for (int j = 0; j < size - 1; j++) {
                auto nodeComma = idComma[path[j]];
                fwrite(nodeComma.c_str(), nodeComma.size(), sizeof(char), fp);
            }
            auto nodeLF = idLF[path[size - 1]];
            fwrite(nodeLF.c_str(), nodeLF.size(), sizeof(char), fp);
        }
    }
