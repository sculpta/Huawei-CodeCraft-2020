...
void dfs(int tid, bool *visit, int head, int cur, int depth, vector<int> &path) {
    ...
    if (depth < 5) {
        ...
        for (int i = 1; i <= pathDetail[tid][cur][401]; ++i) {
            int u1 = pathDetail[tid][cur][401 + i];
            if (!visit[u1]) {
                auto temp = path;
                temp.push_back(u1);
        ...
        for (int j = 1; j <= pathDetail[tid][cur][0] << 1; j += 2) {
            int u2 = pathDetail[tid][cur][j], u1 = pathDetail[tid][cur][j + 1];
    ...
    for (int i = start; i < node_sum - 2; i += THREAD_COUNT) {
        ...
        if (graph[head][0] > 0) {
            for (int j = 0; j < node_sum; ++j) {
                ...
                pathDetail[start][nodes[j]][401] = 0;
            }
            ...
            for (int k = 1; k <= graphIn[head][0]; ++k) {
                ...
                if (u1 > head) {
                    ...
                    for (int l = 1; l <= graphIn[u1][0]; ++l) {
                        int u2 = graphIn[u1][l];
                        if (u2 > head && u2 != u1) {
                            int sum1 = ++pathDetail[start][u2][401];
                            pathDetail[start][u2][401 + sum1] = u1;
