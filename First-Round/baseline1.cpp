...
public:
    ...
    //P2[i][j][k]表示结点i到达结点j，中间经过结点k的路径详情
    //estimate size 28000*100*4
    ...
    void constructP2(){
        P2=vector<unordered_map<int,vector<int>>>(nodeCnt,unordered_map<int,vector<int>>());
        for(int i=0;i<nodeCnt;i++){
            auto &gi=G[i];
            for(int &k:gi){
                auto &gk=G[k];
...
//            }
            ...
            if(!G[i].empty()){
                //可以通过大于head的id返回的
                fill(reachable.begin()+i+1,reachable.end(),-1);
...
//                        if(val.size()==1 && val[0]>i) reachable[j]=0;
//                        else{
...
int main(int argc, char *argv[]) {
    ...
    string testFile = "datasets/" + file + "/test_data.txt";
