...
vector<unordered_map<int, vector<int>>> p3;
...
void createPathDetail() {
    ...
    p3 = vector<unordered_map<int, vector<int>>>(NODE_MAX, unordered_map<int, vector<int>>());
                            ...
                            if (u3 > head && u3 != u1 && u3 != u2) {
                                ...
                                for (int n = 1; n <= graphIn[u3][0]; ++n) {
                                    ...
                                    if (u4 > head && u4 != u1 && u4 != u2 && u4 != u3) {
                                        p3[head][u4].push_back(u3);
                                        p3[head][u4].push_back(u2);
                                        p3[head][u4].push_back(u1);
                                    }
                                }
                            }
                        }
...
void dfs(int tid, bool *visit, int head, int cur, int depth, vector<int> &path) {
    ...
    if (depth < 4) {
        ...
        auto it2 = p3[head].find(cur);
        if (it2 != p3[head].end()) {
            auto paths = (*it2).second;
            int size = paths.size();
            for (int i = 0; i < size;) {
                int u3 = paths[i++];
                int u2 = paths[i++];
