...
void dfs_1(bool *visit, int *isValid, int head, int cur, int w, int depth) {
    ...
    switch (depth) {
        ...
        case 2:
            for (int i = 0; i < (int) graph[cur].size(); i += 2) {
                int v = graph[cur][i], w1 = input_weight[graph[cur][i + 1]], w2 = input_weight[w];
    ...
    switch (depth) {
        case 1:
            for (int i = 0; i < (int) graphIn[cur].size(); i += 2) {
                int u = graphIn[cur][i], w1 = input_weight[graphIn[cur][i + 1]];
        ...
        case 2:
            for (int i = 0; i < (int) graphIn[cur].size(); i += 2) {
                int u = graphIn[cur][i], w1 = input_weight[graphIn[cur][i + 1]], w2 = input_weight[w];
...
void dfs(int loc, bool *visit, int *isValid, int *isTail, vector<unordered_map<int, vector<int>>> &pathDetail, int head,
    ...
    for (int i = 0; i < (int) graph[cur].size(); i += 2) {
        int v = graph[cur][i], w4 = input_weight[w], w3 = input_weight[graph[cur][i + 1]];
        ...
        if (isTail[v] == head) {
            for (int j = 0; j < (int) pathDetail[loc][v].size(); j += 3) {
                ...
                int w1 = input_weight[pathDetail[loc][v][j + 1]];
                int w2 = input_weight[pathDetail[loc][v][j + 2]];
                int w5 = input_weight[w0];
