...
void* magic_train(void* train_info) {
  ...
  while (magic_buf < buf_end) {
    if (*(magic_buf + 6001) == '\n') {
      #define go(op) for (int i = 0; i < 250; ++i) { \
...
void* magic_fun(void *thread_id) {
  ...
  int big = 0;
  while (magic_buf < buf_end) {
    if (*(magic_buf + 2 + max_cof_p * 6) * 10 + *(magic_buf + 3 + max_cof_p * 6) >= 49 * 10 + 48 + 5) {
      sum = INT_MAX;
      magic_buf += 6000;
      ++big;
    } else {
...
void work(const string& file_name, const string& result_file, int pid) {
  ...
  char* ans = (char*)mmap(NULL, test_data_size / 3000, PROT_READ | PROT_WRITE, MAP_SHARED, fd2, 0);
  close(fd2);
